...
public class mopController : MonoBehaviour {
	...
	void Start () {
		...
		playerPos.Set((x2-64)/100,(y2-48)/100,0);
		posList.Add (playerPos);
		Instantiate (mopBackground, (playerPos), Quaternion.identity);

		GameObject newBackground = GameObject.Find("mopBackground(Clone)");
		newBackground.name=("Mop Background " + playerPos);
	...
	void resetPos(){
		...
		x = x - (x % 128);
		y = y - (y % 96);
		playerPos.Set((x+64)/100,(y+48)/100,0);
	...
	void addMopped(Vector3 newPos){
		while (posList.Count > 1) {
			...
			GameObject newMopped = GameObject.Find("postMop(Clone)");
			newMopped.name=("Mopped Square " + statonaryPos);
